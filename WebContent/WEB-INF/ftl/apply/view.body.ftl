...
<div class="cont" style="width:800px;">
	<#if buAccountVO?exists>
		...
		--${.now?date}<br>
		--${"2018/1/3"?date}<br>
		<#if "2018/2/16"?date lte "2018/2/15"?date && buAccountVO?exists>
			A
			<#else>
			B
		</#if>
	...
	<tr>
		<th height="35">禮金序號</th>
			<#if .now?date lte "2018/2/15"?date && buAccountVO?exists>
				<td>
					<input disable type="text" id="giftSno" name="giftSno" value="${accountVO.giftSno!}" autocomplete="off"  style="width:100px" maxlength="10"/>
