...
<table border="0" cellspacing="3" cellpadding="0" class="srchtb"> 
  <tr> 
	   ...
	   <td>
	   		<select id="searchWebsiteCode">
	   				<option value="">全部類型</option>
	   			<#list reportWebsiteCategoryMap?keys as skey>
		  		    <option value="${skey}">${reportWebsiteCategoryMap[skey]}</option>
		  	    </#list>
	   		</select>
	   </td>
...
<form id="excerptFrom" name="excerptFrom" action="reportAdWebsiteDownload.html" method="post">
	...
	<input type="hidden" id="fadSearchWebsiteCode" name="searchWebsiteCode" value="${searchWebsiteCode}">
