...
//		sql.append(" sum((case when r.ad_clk_price_type = 'CPC' then r.ad_clk else r.ad_view end)), ");
//		sql.append(" sum(r.ad_clk_price), ");		
//		sql.append(" sum(r.ad_invalid_clk), ");
//		sql.append(" aa.ad_action_status, ");
//		sql.append(" aa.ad_action_start_date, ");
//		sql.append(" aa.ad_action_end_date, ");
//		sql.append(" r.ad_seq, ");
//		sql.append(" r.customer_info_id, ");
//		sql.append(" r.ad_operating_rule, ");
//		sql.append(" r.ad_clk_price_type, ");
//		sql.append(" r.ad_type, ");
//		sql.append(" aa.ad_action_name, ");
//		sql.append(" IFNULL(TRUNCATE((sum((case when r.ad_clk_price_type = 'CPC' then r.ad_clk else r.ad_view end)) / sum(r.ad_pv)) * 100, 2),0) ad_view_ratings, ");
//		sql.append(" IFNULL(TRUNCATE(SUM(r.ad_clk_price) / (SUM(r.ad_pv)), 2),0), ");
//		sql.append(" r.ad_clk_price_type, ");
//		sql.append(" IFNULL(TRUNCATE(SUM(r.ad_clk_price) / (SUM(r.ad_pv) * 1000), 2),0)thousands_cost, ");
//		sql.append(" IFNULL((SELECT dt.ad_detail_content  ");
//		sql.append(" FROM   pfp_ad_detail dt  ");
//		sql.append(" WHERE  dt.ad_seq = r.ad_seq  ");
...
//		sql.append(" from pfp_ad_report as r,pfp_ad_action aa ");
//		sql.append(" where 1 = 1 ");
//		sql.append(" and r.ad_action_seq = aa.ad_action_seq ");
...
//		sql.append(" and r.ad_group_seq = :adGroupSeq ");
		...
		sql.append(" FROM   pfp_ad_detail dt  ");
		sql.append(" WHERE  dt.ad_seq = r.ad_seq "); 
		sql.append(" AND dt.ad_detail_id = 'video_seconds'), '') "); 
		sql.append(" video_seconds, "); 
		sql.append(" Ifnull((SELECT dt.ad_detail_content "); 
		sql.append(" FROM   pfp_ad_detail dt "); 
		sql.append(" WHERE  dt.ad_seq = r.ad_seq  ");
		sql.append(" AND dt.ad_detail_id = 'video_url'), '')        video_url, "); 
