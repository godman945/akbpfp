	...
	public static void alex(File file) throws Exception {
		...
		for (File f : files) {
			if (f.isFile()) {
	    		...
	    		stringBuffer.setLength(0);
				stringBuffer.append(" /opt/mozjpeg/bin/cjpeg  -quality 75 -tune-ms-ssim   -quant-table 0      -progressive      ").append(f.getAbsolutePath()).append(" > ").append(f.getAbsolutePath().replace(f.getName(), "")).append(f.getName().replace(".jpg", "_resize.jpg"));
				System.out.println(">>>>>>>>>>command:"+stringBuffer.toString());
				process = Runtime.getRuntime().exec(new String[] { "bash", "-c", stringBuffer.toString()  });
				result = IOUtils.toString(process.getInputStream(), "UTF-8");
