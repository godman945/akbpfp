...
//	}
	...
	public PfpAd getPfpAdBySeq(String adSeq) throws Exception {
		List<PfpAd> list = (List<PfpAd>) super.getHibernateTemplate().find("from PfpAd where adSeq = ?", adSeq);
...
//		}
	...
	public List<PfpAd> validAdAd(String adGroupSeq) throws Exception{
		...
		return (List<PfpAd>) super.getHibernateTemplate().find(hql.toString(),adGroupSeq,EnumStatus.Close.getStatusId());
	}
	...
	public List<Object> getAdAdVideoDetailView(PfpAdAdViewConditionVO pfpAdAdViewConditionVO) throws Exception {
		...
		sql.append(" WHEN r.ad_clk_price_type = 'CPC' THEN r.ad_clk "); 
		sql.append(" ELSE r.ad_view "); 
		sql.append(" END)) / Sum(r.ad_pv)) * 100, 2), 0) ad_view_ratings, "); 
