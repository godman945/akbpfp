...
public class IndexAction extends BaseCookieAction {
	...
	public String execute() throws Exception {
		...
		for (PfpUserMemberRef pfpUserMemberRef : pfpUserMemberRefList) {
			if (pfpUserMemberRef.getId().getMemberId().equals(super.getId_pchome())
					&& pfpUserMemberRef.getPfpUser().getPrivilegeId() == EnumPrivilegeModel.ADM_USER.getPrivilegeId()) {
				pfpAngelFlag = true;
				break;
			}
		}
		 ...
		 * if(StringUtils.isNotBlank(this.resultType)){ return this.resultType;
		 * }
		 * 
		 * String ref = request.getHeader("Referer"); log.info("refer: "+ref);
		 * if(StringUtils.isNotBlank(ref)){ log.info("Enum: "
		 * +EnumRedirectAction.CLASSIFIEDS_DEVURL.getAction().toString());
		 * log.info("boolean : "
		 * +ref.contains(EnumRedirectAction.CLASSIFIEDS_DEVURL.getAction().
		 * toString()));
		 * if(ref.contains(EnumRedirectAction.CLASSIFIEDS_DEVURL.getAction().
		 * toString())){ //TODO redirect to directions pages log.info(
		 * "return INPUT to open.html"); return "open"; } }
		 * 
		 * if(StringUtils.isNotBlank(super.getId_pchome())){ // 取使用者資料
		 * List<PfpUserMemberRef> userMemberRefs =
		 * PfpUserMemberRefService.activateUserMemberRef(super.getId_pchome());
		 * 
		 * String ref = request.getHeader("Referer"); // 帳號不存在
		 * if(userMemberRefs.size() <= 0){
		 * 
		 * //從免費刊登進入(無帳號)導open頁 log.info("refer: "+ref);
		 * if(StringUtils.isNotBlank(ref)){
		 * if(ref.contains(EnumRedirectAction.CLASSIFIEDS_DEVURL.getAction().
		 * toString())){ log.info("return INPUT to open.html"); return "open"; }
		 * } }else{ //從免費刊登進入(有pfp帳號)導入原網頁 String url =
		 * request.getRequestURL().toString(); log.info("url == "+url);
		 * redirectUrl = url; return "refer";
		 * 
		 * } }
		 */
...
//		HttpUtil.doGet("https://billing.pchome.com.tw/rmiservice/pv.html", "t=pfp:ck_pfp_index");
	...
	private void getFaqQuestionListVO(String f_id, FaqListVO sFaqListVO) throws Exception {
		...
		for (Iterator<?> g = rootG.elementIterator(); g.hasNext();) {
			...
			if (elementG.isTextOnly()) {
				...
				// 問題列表物件
				// ArrayList<FaqQuestionVO> faqQuestionVOs = new
				// ArrayList<FaqQuestionVO>();
				// 問題列表序號
				int tmpG = 0;
				// 開始讀取問題內容
		...
		for (Iterator<?> i = root.elementIterator(); i.hasNext();) {
			...
			if (element.isTextOnly()) {
				if (element.getName().equals("question")) {
					...
					String content = element.getTextTrim().replaceAll("(?i)(&lt;)", "<").replaceAll("(?i)(&gt;)", ">")
	...
	public String faqSolution() throws Exception {
		...
		// // 設定 faq api 讀取服務中心的網址物件
		// URL sURL = new URL(faqServerSolution + "?channel=show&f_id=" + fid +
		// "&q_id=" + qid);
		// // 設定 dom4j 讀取 XML 的物件
		// SAXReader reader = new SAXReader();
		// // 呼叫 faq api，並設定為 document 物件
	...
	public void setPfdUserMemberRefService(PfdUserMemberRefService pfdUserMemberRefService) {
