...
public class ReportAdTimeAction extends BaseReportAction {
	...
	private List<AdTimeReportVO> resultData = new ArrayList<>(); // 查詢結果
	private List<AdTimeReportVO> resultSumData = new ArrayList<>(); // 查詢結果加總
	...
	public String execute() throws Exception {
		...
		resultData = adTimeReportService.queryReportAdTimeData(vo);
		resultSumData = adTimeReportService.queryReportAdTimeSumData(vo);
		...
		if (!resultData.isEmpty()) {
			...
			for (int i = 0; i < resultData.size(); i++) {
				...
				if ("week".equalsIgnoreCase(viewType)) { // 畫面目前選擇為星期
					content.append("\"" + resultData.get(i).getWeek() + "\",");
				} else {
					content.append("\"" + resultData.get(i).getTime() + "\",");
				}
	...
	public List<AdTimeReportVO> getResultData() {
		return resultData;
	}

	public List<AdTimeReportVO> getResultSumData() {
...
//
//		List<AdTimeReportVO> resultData = adTimeReportService.loadReportDate(EnumReport.REPORT_HQLTYPE_TIME_CHART.getTextValue(),searchTime,searchText, adSearchWay, adShowWay, adPvclkDevice, customerInfoId, adOperatingRule, startDate, endDate, -1, -1);
//
...
//
//		List<AdTimeReportVO> resultSumData = adTimeReportService.loadReportDate(EnumReport.REPORT_HQLTYPE_TIME_COUNT.getTextValue(), searchTime, searchText, adSearchWay, adShowWay, adPvclkDevice, customerInfoId, adOperatingRule, startDate, endDate, -1, -1);
//
//		int totalPageSize = resultSumData.size();
//
//		List<AdTimeReportVO> resultData = adTimeReportService.loadReportDate(EnumReport.REPORT_HQLTYPE_TIME.getTextValue(), searchTime, searchText, adSearchWay, adShowWay, adPvclkDevice, customerInfoId, adOperatingRule, startDate, endDate, page, pageSize);
//
