...
public class ReportAdWebsiteAction extends BaseReportAction {
	...
	private IPfbxWebsiteCategoryService pfbxWebsiteCategoryService; // 取得網站類型下拉選單資料
	...
	private List<AdWebsiteReportVO> resultData = new ArrayList<>(); // 查詢結果
	private List<AdWebsiteReportVO> resultSumData = new ArrayList<>(); // 查詢結果加總
	...
	public String execute() throws Exception {
		...
		resultData = adWebsiteReportService.queryReportAdWebsiteData(vo);
		resultSumData = adWebsiteReportService.queryReportAdWebsiteSumData(vo);
	...
	private void makeDownloadReportData() throws Exception {
		...
		content.append("狀態,廣告活動,廣告分類,網站分類,");
		...
		if (!resultData.isEmpty()) {
			...
			for (int i = 0; i < resultData.size(); i++) {
				...
				content.append("\"" + resultData.get(i).getWebsiteCategoryName() + "\",");
...
////		return adWebsiteReportService.getWebsiteCategoryMap();
//	}
	...
	public List<AdWebsiteReportVO> getResultData() {
		return resultData;
	}

	public List<AdWebsiteReportVO> getResultSumData() {
