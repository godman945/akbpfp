...
public class ReportCampaginAction extends BaseReportAction {
	...
	public String flashDataDownLoad() {
		...
		List<AdCampaginReportVO> resultChartData = adActionReportService.queryReportAdCampaginChartData(chartVo);
		
		Map<Date, Float> flashDataMap = new HashMap<>();
		
		for (int i = 0; i < resultChartData.size(); i++) {
			AdCampaginReportVO vo = resultChartData.get(i);
...
//		}
//		if(hasPfpCodeflag){
//			optionSelect="每日花費,曝光數,互動數,互動率,單次互動費用,千次曝光費用,費用,轉換次數,總轉換價值,轉換率,平均轉換成本,廣告投資報酬率";	
//		}else{
//			optionSelect="每日花費,曝光數,互動數,互動率,單次互動費用,千次曝光費用,費用";
//		}
...
//			int dataNumber = 1;
//			for(String s:sl){
//				if(dataNumber == 7 ||dataNumber == 11 || dataNumber == 12 || dataNumber == 13 || dataNumber == 15 || dataNumber == 17){
