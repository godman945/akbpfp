					...
					<#else>
						<#if vo.adOperatingRule =='PROD'>
								...
								<iframe height="250" width="300" class="akb_iframe" scrolling="no" frameborder="0" marginwidth="0" marginheight="0" vspace="0" hspace="0" id="pchome8044_ad_frame1" allowtransparency="true" allowfullscreen="true" 
									...
									&logoType=${vo.logoType!}
									&logoText=${vo.logoText!}
									&logoBgColor=${vo.logoBgColor!}
									&logoFontColor=${vo.logoFontColor!}
									&prodLogoType=${vo.prodLogoType!}
									&adbgType=${vo.adbgType!}
									&imgProportiona=${vo.imgProportiona!}
									&userLogoPath=${vo.userLogoPath!}
									&realUrl=${vo.realUrl!}
									&previewTpro=c_x05_pad_tpro_0145
									&saleImg=${vo.saleImg!}
									&posterType=${vo.posterType!}
								...
								<div style="margin-top: 5%;word-break:break-all;">
									廣告名稱：${vo.adName!}<br>
									連結網址：<a href="<#if vo.showUrl?contains('http')>${vo.showUrl!}<#else>http://${vo.showUrl!}</#if>" target="_blank" title="">${vo.showUrl!}</a>
								</div>
								</div>
							<#else>
