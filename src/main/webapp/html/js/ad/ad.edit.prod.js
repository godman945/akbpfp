...
function initSaveData(){
	...
	if(JSON.parse($("#saveSaleEndImg").text()).length > 0){
		$(".advancedtbbx")[0].setAttribute("class", "advancedtbbx open");
	}
	if(JSON.parse($("#saveSaleImg").text()).length > 0){
		$(".logobannerbx")[0].setAttribute("class", "logobannerbx open");
	}
...
function initFancyBoxHtml(){
	...
	for (var key in salesIframewp.data) {
		...
		fancyboxSaleEndHtml.push('<li>');
		fancyboxSaleEndHtml.push('	<div class=\"upload-lab\" style=\"display:block;\" onclick=\"fileLoad(this)\">');
		fancyboxSaleEndHtml.push('		<label for="" class="custom-file-upload" style="height:auto;">');
...
function checkUploadRule(file_data){
	...
	readFile(file_data.files[0], function(e) {
		...
		image.onload = function () {
            ...
            if(fileExtensionName.indexOf("JPEG") >= 0 || fileExtensionName.indexOf("PNG") >=0 || fileExtensionName.indexOf("GIF") >=0){
	...
	if(submitFlag){
		...
		if(confirm(alt)) {
			$.blockUI({
		        ...
		        onBlock: function() {
		        	var adSeq = $("#adSeq").val();
		        	var uploadLogSave = JSON.stringify(saleEndImgUpload);
		        	var uploadLogoLogSave = JSON.stringify(saleImgUpload);
