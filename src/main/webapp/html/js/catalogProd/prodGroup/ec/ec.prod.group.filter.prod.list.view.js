...
var defaultTemplate = 
    ...
    '<div class="txt-inlineblock level3">'+
       ...
       '<div class="input-text" data-level="5">'+
       		'<input class="tags" type="text" name="" maxlength="20" value="">'+
...
//						console.log(event.handleObj.type);
    ...
    //商品組合至少須包含一組篩選條件，第一組條件改為無法移除
    deleteFirstFilterCondition();
});
...
//商品組合至少須包含一組篩選條件，第一組條件改為無法移除
function deleteFirstFilterCondition(){
	$(".filter-wrap .filter-group").each(function(index,obj) {
		 if(index == 0){
			 $(obj).find('.icon-kill').remove();
		 }
	});
}
