...
function viewDetail(dataObj){
  ...
  $.fancybox(
		  ...
		  '				</div>'+
		  '				<div class="weburl"> <a href='+$(dataObj).find('.prodcard-infobox').find('.g4').find('#weburlDetailData').children().attr('href')+' target="_blank" >'+$(dataObj).find('.prodcard-infobox').find('.g4').find('#weburlDetailData').children().attr('href')+'</a> </div>'+
		  '			</div>'+
		  '		</div>'+
		  '	</div>'+
		  '</div>',
    	...
    	if (response.status=="ERROR"){
	    	...
	    	$.each(response.prodList, function(index, list){
	    		...
	    		$.each(list, function(key, val){
	    			...
	    			if(key == "ecVerifyRejectReason"){
	    				ecVerifyRejectReason = val;
	    			}
	    		});
	    			if (ecStatus == "1"){
	    			...
	    			if (ecCheckStatus == "2"){
	    				...
	    				tempHtml += "		<em style='display:none'>"+ecVerifyRejectReason+"</em> ";
	    				tempHtml += "	</div> ";
	    				tempHtml += "</div> ";
	    			}else{
