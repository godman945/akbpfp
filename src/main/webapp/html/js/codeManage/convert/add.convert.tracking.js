...
$(document).ready(function() {
	...
	//新增再行銷追蹤
	$('#addConvertTracking').on('click',function(){
		if (checkData() == true){
			prepareData();
			addConvertTrackingAjax();
		}
	})
	
	//code1複製成功
	$('#copyCode1').on('click',function(){
	  $("#copyCodeSuccess1").css('display','');
	});
	
	//code2複製成功
	$('#copyCode2').on('click',function(){
	  $("#copyCodeSuccess2").css('display','');
	});
...
function init(){
	//互動後轉換追溯天數
	$('#clickRangeDate1').trigger("click");
	clickRangeDate=1;
	
	//瀏覽後轉換追溯天數
	$('#impRangeDate1').trigger("click");
	impRangeDate=1;
...
//新增轉換追蹤
function addConvertTrackingAjax(){
	$.ajax({
		type : "post",
		dataType : "json",
