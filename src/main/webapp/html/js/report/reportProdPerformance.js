...
$(document).ready(function() {
	...
	var hrefName = $(".tableform_breadcrumbs span a").attr('href');
	$("div.submn a[href$='" + hrefName + "']").css('color', '#222');
...
function processQueryAjax(changePageNo) {
	$.ajax({
	    type : "post",
		url : "reportProdPerformanceAjaxTable.html",
	    data: {
	    	  "pfpCustomerInfoId" : $('#pfpCustomerInfoId').val(),
	    	  "adSeq" : $('#adSeq').val(),
...
function processDownloadReport() {
	...
	$("#excerptFrom").attr("action", "reportProdPerformanceDownload.html");
	$("#excerptFrom").submit();
}
...
function showHighChart(){
	var dataArray;
	$.ajax({
		url : "reportProdPerformanceAjaxChart.html",
		type : "POST",
		dataType:'json',
		async: false,
		data : {
...
//			console.log(respone);
	...
	switch(selectType){
		...
		case "click":
			titleName = "商品點選數(次)";
			selectTypeName = "商品點選數";
			selectSuffix = "次";
			break;
		case "ctr":
			titleName = "商品點選率(%)";
			selectTypeName = "商品點選率";
			selectSuffix = "%";
			decimals = 2;
			break;
	}
